...
- [ ] Revisar el modo máquina de escribir (typewriter, `AppearingCredits.tsx`): a veces el texto aparece directamente sin verse el efecto de tecleo, y la velocidad de escritura resulta confusa. Diagnosticar el caso en que no se reproduce el efecto y revisar/exponer la velocidad. Pedido 2026-06-23.
- [ ] Mover el control de bucle (`Switch` de `config.loop`, hoy en `ConfigPanel.tsx:600`, sección de animación de aparición) junto a los controles de reproducción (`CreditPreview.tsx`). Pedido 2026-06-23.
- [ ] Control de blur (desenfoque) del texto en sí (`filter: blur(...)` sobre el texto, distinto del `textShadowBlur` de la sombra que ya existe). Valorar global + override por item. Pedido 2026-06-23.
- [ ] Drag and drop de items no funciona. Verificado 2026-06-23: en `CreditEditor.tsx` el icono `GripVertical` (línea 100) es solo decorativo; no hay `draggable`/`onDragStart`/`onDrop` ni se llama a `reorderItems` del store (el reducer existe pero la UI no lo cablea). Implementar el reordenamiento por arrastre. Pedido 2026-06-23.
